...
from lib.demo_data.watership_down_seed_data import books_list, characters_list, locations_list, parts_list, chapters_list, character_groups_list
...
def run_demo_data():
    if len(sys.argv) > 1 and sys.argv[1] == "demo-data":
        ...
        add_items("Watership Down", "Part", parts_list)
        add_items("Watership Down", "Chapter", chapters_list)
        add_items("Watership Down", "Book", books_list)
        add_items("Watership Down", "Location", locations_list)
        add_items("Watership Down", "Character Group", character_groups_list)
